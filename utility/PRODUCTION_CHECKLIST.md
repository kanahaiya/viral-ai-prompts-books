...
**CRITICAL: Enable cURL on the server first**
- [ ] Contact hosting provider (Hostinger) to enable PHP cURL extension
- [ ] Verify cURL is enabled: test page or hosting control panel should show cURL in enabled PHP extensions
- [ ] If self-hosted: ensure `php.ini` has `extension=curl` uncommented and PHP is rebuilt if necessary
